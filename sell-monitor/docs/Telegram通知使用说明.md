...
# 如果 api.telegram.org 直连不通，可填写本地代理
SELL_MONITOR_TELEGRAM_PROXY=http://127.0.0.1:7890
```

说明：

- `SELL_MONITOR_TELEGRAM_PROXY` 是新加的可选项
- 如果你的机器能直连 Telegram，可以不填
- 如果你在本机开了 Clash、v2rayN、Nekoray 之类代理，填对应本地 HTTP 代理地址即可
...
### 7.3. 一直收不到消息，但程序没报 token/chat id 错

这通常不是配置错，而是机器到 `api.telegram.org:443` 不通。

这种情况下：

1. 先确认你本机是否开了可用代理
2. 若有本地 HTTP 代理，在配置里补：
