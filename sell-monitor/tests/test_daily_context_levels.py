...
class DailyContextLevelsTest(unittest.TestCase):
    def test_daily_context_keeps_ab_zones_and_c_resistance(self) -> None:
        zones = [
            ...
            PriceZone("d", "1d", 7, 8, 3, ZoneLevel.C, ["resistance"], 1),
            PriceZone("e", "1d", 6, 7, 1, ZoneLevel.D, ["resistance"], 1),
        ]

        filtered = _daily_ab_zones(zones)

        self.assertEqual(["a", "b", "d"], [zone.name for zone in filtered])
