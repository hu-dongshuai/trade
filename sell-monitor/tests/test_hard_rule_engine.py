...
class HardRuleEngineTest(unittest.TestCase):
    ...
    def test_third_upper_wick_with_ma20_high_volume_break_exits(self) -> None:
        decision = evaluate_hard_rules(
            symbol="002241",
            current_price=25.0,
            position=Position("002241", 20.0, 100),
            ...
            signals=[
                Signal("third_dangerous_upper_wick", 3, True, "出现第三根危险上影线"),
                Signal("m15_ma20_high_volume_break", 2, True, "放量跌破15分钟MA20"),
            ],
        )

        self.assertIsNotNone(decision)
        self.assertEqual(Action.EXIT_ALL, decision.action)
        self.assertIn("放量跌破15分钟MA20", decision.reasons)
